...
public class registerClicked extends AbstractAction {
   ...
   public registerClicked (JCheckBox platingDir, ReconstructionModel root,
   MechModel mechModel) {
   ...
   public void actionPerformed (ActionEvent evt) {
      ...
      String masterMandiblePath =
         homeParent
         + "/artisynth_istar/src/artisynth/istar/Prisman/stl/MasterMandible.stl";
      PolygonalMesh masterMandibleMesh =
         meshHelper.readMesh (masterMandiblePath, "MasterMandible.stl");
      PolygonalMesh mandibleMesh;
      if (Assist
         .GetMesh (mechModel, "Mandible").getVertices ().size () > 2000) {
         PolygonalMesh DecimatedMandible =
            Assist.GetMesh (mechModel, "Mandible").copy ();
         while (DecimatedMandible.getVertices ().size () > 2000) {
            MeshUtilities.quadricEdgeCollapse (DecimatedMandible, 1000);
            if (root.DEBUG) {
               System.out
                  .println (
                     "Number of Vertices for Registration "
                     + DecimatedMandible.getVertices ().size ());
            }
         }
         mandibleMesh = new PolygonalMesh (DecimatedMandible);
      }
      else {
         mandibleMesh =
            new PolygonalMesh (Assist.GetMesh (mechModel, "Mandible"));
      }
      ...
      String guidePath =
         homeParent
         + "/artisynth_istar/src/artisynth/istar/Prisman/stl/MasterGuidesThin.stl";
      ...
      mandibleMesh.computeCentroid (centroidPatient);
      ...
      PolylineMesh PlateMesh =
         (PolylineMesh)Assist.GetNonPolyMesh (mechModel, "Plate");
