...
import artisynth.istar.Atabak.AtabakRender.*;

public class Optimize implements Command{
   
   Assist Assist = new Assist ();
   MechModel mechModel = null;
   ReconstructionModel root = null;  
   PolygonalMesh ResectedMandible1 = new PolygonalMesh ();
   PolygonalMesh ResectedMandibleTest = new PolygonalMesh ();
   PolygonalMesh NativeMandible = new PolygonalMesh ();
   ...
   List<PolygonalMesh> donorSegmentMeshesShort = new ArrayList<PolygonalMesh> ();
   List<FixedMeshBody> donorSegmentBodiesShort = new ArrayList<FixedMeshBody> ();
   ImprovedFormattedTextField DonorDistanceProx;
   ImprovedFormattedTextField DonorDistanceDis;   
   // these last 2 instance variables should be added to the constructor because the surgeon needs to insert these as input 
   ...
   PrepDonorFirst PrepDonor1 = new PrepDonorFirst ();
   PrepDonorSecond PrepDonor2 = new PrepDonorSecond();
   // manidble ---------
   List<PolygonalMesh> mandiblePlanes = new ArrayList<PolygonalMesh> ();
   
   //-----------------------------------------------------------------
   // instance variables for the segment cutting of the donor part:
   List<Point3d> spoints = new ArrayList<Point3d> ();
   
   //quaternions  
   Quaternion FirstPlanequat = new Quaternion();
   
   
   public Optimize (ReconstructionModel root, MechModel mechModel,
      ImprovedFormattedTextField DonorDistanceProx,
      ...
      this.root = root;
      this.ResectedMandible1 = Assist.GetMesh (mechModel, "Mandible").copy ();
      this.NativeMandible = Assist.GetMesh (mechModel, "BackupMandible").copy ();
   ...
   public void execute () {
      RigidBody clippedDonorMeshBody = new RigidBody ("ClippedDonor");
      PolygonalMesh clippedDonorMesh = new PolygonalMesh ();
      List<Object> returnObjects = PrepDonor1.prepareDonorv2 (disPoint, Float.parseFloat (DonorDistanceProx.getText ()),
         Float.parseFloat (DonorDistanceDis.getText ()), DonorCuttingPlanes,
         clippedDonorMesh, Assist.GetMesh (mechModel, "Donor"), Assist.GetMeshBody (mechModel, "Donor"));
    ...
    // after cutting the end parts, We are going after cutting out the segments ----------------------------------------------
    //--------------------------------------------------------------------------------------------------------------------------
    //--------------------------------------------------------------------------------------------------------------------------
      ...
      // donor in a box and calculating the sortedAxes
         ...
         Vector3d normProx = new Vector3d (DonorCuttingPlanes[0].getNormal (0).copy ());
         normProx.normalize ();
         ...
         FirstPlanequat.set (poseplane1.R);
         System.out.println("plane1 quaternion" + FirstPlanequat);
...
//                  (15 + scaleValue) * (1 - theta / Math.PI);
         ...
         //----------------------------------------------------------------------------------------------------------------------
         //----------------------------------------------------------------------------------------------------------------------
         ResectedMandibleTest = Assist.GetMesh (mechModel, "Resection");
         PolygonalMesh Test = new PolygonalMesh();
         Test = MeshFactory.createPlane (90, 90, 10, 10);
         FixedMeshBody Test1 = new FixedMeshBody ("Test FixedMeshBody", Test);
         RigidTransform3d testpose = Test1.getPose ();
         Vector3d normalTest1 = Test.getNormal (0);
         Point3d centofTest = new Point3d();
         //mandiblePlanes.get (0).computeCentroid (centofTest);
         //centofTest.sub (new Vector3d(0,0,6));
         ResectedMandibleTest.computeCentreOfVolume (centofTest);

         meshHelper
         .createPlane (
            normalTest1, new Vector3d (0,0,1), centofTest,
            testpose, Test);
         RigidBody TestBody = new RigidBody("testmesh");
         TestBody.setSurfaceMesh (Test);
         TestBody.transformGeometry (testpose);
         mechModel.addRigidBody (TestBody);
         ArrayList<IntersectionContour> contours =  smi.findContours (Test, ResectedMandibleTest);
         ArrayList<Point3d> contourPoints = new ArrayList<Point3d>();
         
         List<Particle> Particles = new ArrayList<Particle>();
         int i =0;
         for (IntersectionContour contour : contours) {
            // check points on each contour
            for (Point3d p : contour) {
               contourPoints.add(p);
//               Particles.add (new Particle (0.1, p.x, p.y, p.z));
//               Particles.get (i).setDynamic (false);
//               RenderProps.setSphericalPoints (Particles.get (i), 0.6, Color.RED);
//               mechModel.addParticle (Particles.get (i));  
               i++;
            }
         }
         
         int numberOfSubdivisions = 300;
         NumericList NativeNumericList= new NumericList (3);
         Interpolation cubic = new Interpolation ();
         cubic.setOrder (Order.Linear);
         int size = contourPoints.size();
         System.out.println("size" + size);
         Point3d[] points = new Point3d[size];
         for (i = 0; i < size; i++) {
            points[i] = contourPoints.get(i);
            NativeNumericList.add (points[i], i); //* numberOfSubdivisions/ (size - 1)
         }
         ...
         for (i = 0; i < numberOfSubdivisions; i++) {
            ...
            NativeNumericList.interpolate (interpolatedVectors[i], i);
         }
         System.out.println("NativeNumericList size" + NativeNumericList.getNumKnots ());
         NumericList InterpolatedNumericList= new NumericList (3);
         // Adding Results of interpolation to the numericList
         for (i = 0; i < numberOfSubdivisions; i++) {
            InterpolatedNumericList.add (interpolatedVectors[i], i);
         }
         
         int newSize = InterpolatedNumericList.getNumKnots ();
         System.out.println("newSize" + newSize);
         Point3d[] interpolatedPoints = new Point3d[newSize];
         int[][] indices = new int[newSize - 1][2];

         Iterator<NumericListKnot> itr = InterpolatedNumericList.iterator ();
         ...
         PolylineMesh sphericalPolyLine = MeshFactory.createSphericalPolyline (50, 50, 50);
         PolylineMesh ContourMesh = new PolylineMesh ();
         ContourMesh.addMesh (sphericalPolyLine);
         ContourMesh.set (interpolatedPoints, indices);
         FixedMeshBody Contour = new FixedMeshBody ("Contour", ContourMesh);
         mechModel.addMeshBody (Contour);
         RenderProps.setLineStyle (Contour, LineStyle.CYLINDER);
         
         List<Particle> Particles1 = new ArrayList<Particle>();
         i=0;
         for (Point3d p : interpolatedPoints) {
            Particles1.add (new Particle (0.1, p.x, p.y, p.z));
            Particles1.get (i).setDynamic (false);
            RenderProps.setSphericalPoints (Particles1.get (i), 0.6, Color.BLUE);
            mechModel.addParticle (Particles1.get (i));  
            i++;
         }
         
            
         AtabakRender  renderable = new AtabakRender ();
         DrawBase drawable;
         drawable = new DrawAxes();

         mechModel.addRenderable (drawable);


//         
//         RenderProps props = (new Point3d()).createRenderProps();
