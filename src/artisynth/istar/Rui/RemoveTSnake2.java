...
public class RemoveTSnake2 extends RootModel {
...
//   FemMeshComp teethTeller;
  ...
  // JButton undoTR;
   ...
   public void build(String[] args) throws IOException {
      ...
      String targetMeshName = "proposedT48/013-Mandible.stl";   //  7
...
//      
      ...
      manualFix.setLayout (new GridLayout (0, 3));
   ...
   private class TimeCheck extends ControllerBase {
      ...
      public void apply (double t0, double t1) {
         ...
         if(t1 == runTime) {
            ...
            Intersector myIntersector = new Intersector (target, cMesh, 0, Color.red);
            ArrayList<ArrayList<Vertex3d>> keyVertsT = new ArrayList<ArrayList<Vertex3d>>();            
            myContours = myIntersector.getContours ();
            CSGContourProcess.projection (myContours, keyVertsT, target, cMesh, 45);
            System.out.println("myContours0 "+ myContours.size ());
            target.setVertexColoringEnabled ();
            for(int i=0; i<keyVertsT.size (); i++) {
               for(int j=0; j<keyVertsT.get (i).size (); j++) {
                  keyVerts.add (keyVertsT.get (i).get (j));                  
               //   target.setColor(keyVertsT.get (i).get (j).getIndex (),Color.orange);  
               }
            }
            
            System.out.println("keyVerts1 "+ keyVerts.size ()); 
            for (int j=0; j<iterSnake; j++) { 
             
               SnakeContour2.checkSDF(keyVerts,target, minY); 
               ctrVerts = SnakeContour2.snakeCurv(keyVerts, ctrVerts, target, minY);
               if(ctrVerts == null) {
            ...
            System.out.println("ctrVerts "+ ctrVerts.size ()); 
            
            
           // timer.checkpoint("End snake");
            
            if(ctrVerts != null) {
               SnakeContour2.removeSmallCircles(keyVerts, ctrVerts, target);
